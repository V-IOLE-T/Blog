...
function createPATHAugmentationFields(plugin, container_element, path_augmentations) {
    ...
    new obsidian.Setting(container_element)
        .setName(`将目录添加到环境变量 ${path_variable_name}`)
        .setHeading()
        ...
        .setIcon("bullet-list")
        .setTooltip(`显示当前${path_variable_name}内容（无任何添加）。`)
        .onClick(() => {
        ...
        .addExtraButton(button => button
        .setIcon("help")
        .setTooltip(`文档：添加到 ${path_variable_name} 环境变量`)
...
class OutputWrapperSettingsModal extends SC_Modal {
    ...
    onOpen() {
        ...
        new obsidian.Setting(container_element)
            .setName("内容")
            .setDesc("使用 {{output}} 作为接收 Shell 命令输出文本的占位符，其他变量也可用。")
...
class CustomShellSettingsModal extends SC_Modal {
    ...
    onOpen() {
        ...
        new obsidian.Setting(containerElement.createDiv({ attr: { class: "SC-setting-group" } }))
            .setName("Shell 的操作系统")
            .setDesc("如果 shell 进行虚拟化、用作子系统或以其他方式模拟所选主机操作系统之外的另一个操作系统，请在此处选择它。这用于使目录路径等正常工作。")
    ...
    createBinaryPathSetting(containerElement) {
        new obsidian.Setting(containerElement)
            .setName("可执行文件路径")
            .setDesc("这应该只包含一个目录和一个文件名（或只是一个文件名），而不是任何可能的命令行选项/参数。它们将在下面进行配置。")
        ...
        const updateBinaryPathWarning = () => {
            ...
            if ("" === binaryPath) {
                // No binary path.
                binaryPathWarningDescription.setDesc("注意：尚未定义二进制可执行文件路径。");
                ...
                if (actualBinaryPathExists || appendedBinaryPathExists) {
                    ...
                    if (this.getCustomShellConfiguration().host_platform === getOperatingSystem()) {
                        ...
                        if (path__namespace.isAbsolute(binaryPath)) {
                            ...
                            binaryPathWarningDescription.setDesc(`${notExistsText} 路径是绝对路径，这很好，但也许里面有拼写错误？${testShellText}`);
                        }
                        else {
    ...
    createHostPlatformWindowsSpecificSettings(containerElement) {
        ...
        new Setting(containerElement)
            .setName("Windows 专用")
            .setHeading()
            .setDesc("这些设置仅在你的工作站使用 Windows 时生效。")
        ;
        */
        ...
        const quoteShellArgumentsSetting = new obsidian.Setting(containerElement)
            ...
            .addExtraButton(button => button
            .setIcon("help")
            .setTooltip("文档：Windows：引用 shell 参数")
    ...
    createPathTranslatorField(containerElement) {
        ...
        new obsidian.Setting(containerElement)
            .setDesc("该函数不应引起副作用！它不得更改其外部的任何数据。尽量保持函数简短，因为可能的错误很难检查。永远不会为相对路径调用该函数。")
            .setClass("SC-full-description");
...
class CustomVariableModel extends Model {
    ...
    _createSettingFields(instance, container_element) {
        ...
        new obsidian.Setting(container_element)
            .setName("变量名")
            .setDesc("至少包含 1 个字符。允许使用 a-z 字母、0-9 数字和下划线 _。")
        ...
        new obsidian.Setting(container_element)
            .setName("描述")
            .setDesc("会和变量名一起出现在自动补全列表中；如果使用它，也会显示在“自定义变量”面板里。")
            .addText(text => text
            ...
            case "password": {
                ...
                setting.addText((text_component) => {
                    ...
                    if (isPassword) {
                        ...
                        text_component.setPlaceholder("密码处理时不加密。");
                    }
                });
    ...
    _createSettingFields(prompt_field, container_element) {
        ...
        const setting_group = {
            ...
            target_variable_setting: new obsidian.Setting(setting_group_element)
                .setName("目标变量")
                .setDesc("输入的值将存储到这里，你可以在 Shell 命令中使用这个变量。")
                .addDropdown(dropdown => dropdown
    ...
    createTypeSpecificSettingFields(promptField, containerElement, showCorrectDefaultValueField) {
        ...
        switch (promptFieldConfiguration.type) {
            ...
            case "toggle":
                ...
                new obsidian.Setting(containerElement)
                    .setName("开启时的结果")
                    .setDesc("如果选中切换，目标变量将具有什么值。如果默认值（上面定义）与此值匹配，则默认情况下会检查切换。匹配不区分大小写。")
                    .addText(textComponent => textComponent
                ...
                new obsidian.Setting(containerElement)
                    .setName("关闭时的结果")
                    .setDesc("当开关未勾选时，目标变量将采用的值。这里和上方都可以使用 {{variables}}。")
                    .addText(textComponent => textComponent
...
class PromptSettingsModal extends SC_Modal {
    ...
    onOpen() {
        ...
        const title_setting = new obsidian.Setting(title_and_description_group_element)
            ...
            .setTooltip("尝试提示而不执行任何 shell 命令。")
            .setIcon("run-command")
            .onClick(() => {
            // "Dry run" the Prompt
        ...
        new obsidian.Setting(title_and_description_group_element)
            .setName("描述")
            .setDesc("显示在提示标题和字段之间。Description 和 Title 都支持 {{variables}}。")
            .addTextArea(textarea => textarea
        ...
        new obsidian.Setting(container_element)
            .setName("在提示中预览 Shell 命令")
            .setDesc("如果打开此选项，提示符将显示其中包含变量名称的可执行 shell 命令，并突出显示将受提示符中输入的值影响的变量。")
            .addToggle(toggle => toggle
        ...
        new obsidian.Setting(container_element)
            .setName("字段")
            .setDesc("提示：你可以在“字段标签”“默认值”“Description”等位置使用 {{variables}}。");
        const prompt_field_model = getModel(PromptFieldModel.name);
...
class ShellCommandSettingsModal extends SC_Modal {
    ...
    async tabGeneral(container_element) {
        ...
        new obsidian.Setting(stdinSettingContainer)
            .setDesc("可以包含 {{variables}} 和/或静态文本。")
            .addTextArea(textareaComponent => {
            textareaComponent
        ...
        // Shell command id
        new obsidian.Setting(container_element)
            .setDesc(`Shell 命令 ID：${this.shell_command_id}`)
            .addExtraButton(button => button
            .setIcon("documents")
            .setTooltip(`将 ${this.shell_command_id} 复制到剪贴板。`)
            .onClick(() => {
        ...
        if (this.t_shell_command.canAddToCommandPalette()) {
            ...
            new obsidian.Setting(container_element)
                .setDesc(`Obsidian 命令面板 ID：${obsidian_command_id}`)
                .addExtraButton(button => button
                .setIcon("documents")
                .setTooltip(`将 ${obsidian_command_id} 复制到剪贴板。`)
                .onClick(() => {
    ...
    async tabOutput(container_element) {
        ...
        new obsidian.Setting(container_element)
            ...
            .setTooltip("文档：输出处理模式"));
        // Order of output channels
        new obsidian.Setting(container_element)
            .setName("stdout/stderr 输出顺序")
            .setDesc("当输出同时包含错误和正常输出时，应该先出现哪一个？ （仅当“输出处理”为“等待完成”时才重要）。")
            .addDropdown(dropdown => dropdown
            .addOptions({
    ...
    async tabEvents(container_element) {
        ...
        new obsidian.Setting(debouncingSettingsContainer)
            .setName("防抖（实验性）")
            .setHeading()
            .setDesc("如果启用，事件无法执行此 shell 命令的多个并发（或太相邻）执行。去抖动不会影响标有 ")
            .addExtraButton(helpButton => helpButton
            .setIcon("help")
            .setTooltip("文档：事件 - 去抖动")
        ...
        const defineDebounceAdditionalSettings = () => {
            ...
            if (this.t_shell_command.isDebouncingEnabled()) {
                ...
                new obsidian.Setting(debounceAdditionalSettingsContainer)
                    .setName("冷却时长（秒）")
                    .setDesc("如果你只想防止并发执行，而不需要额外冷却时间，可以将其设为 0。")
    ...
    async tabVariables(containerElement) {
        ...
        new obsidian.Setting(containerElement)
            .setName("变量默认值")
            .setDesc("的事件 某些变量在某些情况下可能无法访问，例如当没有文件窗格聚焦时，{{file_name}} 不可用。您可以定义当变量不可用时将使用的默认值。")
            .setHeading();
    ...
    newAnsiCodeConversionSetting(containerElement, description) {
        const ansiCodeSetting = new obsidian.Setting(containerElement)
            .setName("检测输出中的颜色、字体样式等（ANSI 代码）")
            .setDesc(description);
        ...
        ansiCodeSetting.addExtraButton(helpButton => helpButton
            .setIcon("help")
            .setTooltip("文档：使用 ANSI 代码输出文本样式")
    ...
    newDefaultShellCommandContentSetting(containerElement, onChange) {
        ...
        settingGroup.name_setting.setDesc("用于那些没有定义默认 Shell 命令的操作系统。");
...
class SC_MainSettingsTab extends obsidian.PluginSettingTab {
    ...
    createSearchField(container_element) {
        ...
        const search_setting = new obsidian.Setting(search_container)
            ...
            .setDesc("查找 Shell 命令的别名、命令、ID 和图标。")
            ...
            if ("" == search_term) {
                ...
                switch (count_matches) {
                    case 0: {
                        search_setting.setName("无匹配");
                        break;
                    }
                    case 1: {
                        search_setting.setName("1 个匹配");
                        break;
                    }
                    default: {
    ...
    async tabEvents(container_element) {
        ...
        new obsidian.Setting(container_element)
            .setName("启用事件")
            .setDesc("如果需要，这是一个立即关闭所有事件的快捷方式。")
            .addToggle(toggle => toggle
    ...
    async tabVariables(container_element) {
        ...
        new obsidian.Setting(container_element)
            .setName("在命令面板和菜单中预览变量")
            .setDesc("如果打开，当您在命令选项板中查看命令并右键单击上下文菜单（如果使用）时，变量名称将替换为其实时值。确保您的命令使用正确值的好方法。")
        ...
        new obsidian.Setting(container_element)
            .setName("显示自动补全菜单")
            .setDesc("如果打开，当您开始写入 {{variable}} 名称时，会显示一个下拉菜单，显示匹配的变量及其指令。还允许在 autocomplete.yaml 文件中定义自定义建议 - 请参阅文档。")
            .addToggle(checkbox => checkbox
        ...
        new obsidian.Setting(container_element)
            ...
            .addExtraButton(extra_button => extra_button
            .setIcon("pane-layout")
            .setTooltip("打开一个显示所有自定义变量及其值的窗格。")
            .onClick(() => {
        ...
        new obsidian.Setting(custom_variable_container)
            .setName("自定义变量值变化时显示通知")
            .setDesc("例外：通过提示手动修改值时不会显示通知。")
            .addDropdown(dropdownComponent => dropdownComponent
            .addOptions({
    ...
    async tabEnvironments(container_element) {
        ...
        new obsidian.Setting(container_element)
            .setName("自定义 Shell")
            .setDesc("在这里定义例如 WSL（Windows Subsystem for Linux）、MinGW-w64（Git Bash）或 Wine。")
            .setHeading() // Make the "Custom shells" text bold.
            .addExtraButton(extra_button => extra_button
            .setIcon("help")
            .setTooltip("文档：自定义 shell")
            .onClick(() => {
    ...
    async tabOutput(container_element) {
        ...
        new obsidian.Setting(container_element)
            ...
            .setDesc("如果 shell 命令的输出定向到剪贴板，还会在右上角的弹出框中显示输出。这有助于注意剪贴板中插入的内容。")
            .addToggle(checkbox => checkbox
