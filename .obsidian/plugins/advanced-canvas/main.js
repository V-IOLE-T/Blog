...
var DebugHelper = class {
  ...
  logEfficiency() {
    ...
    if (!canvas) return;
    console.log("节点已添加 效率：", this.nodeAddedCount / canvas.nodes.size);
    console.log("节点已更改 效率：", this.nodeChangedCount / canvas.nodes.size);
    console.log("边已添加 效率：", this.edgeAddedCount / canvas.edges.size);
    console.log("边已更改 效率：", this.edgeChangedCount / canvas.edges.size);
  }
...
function styleAttributeValidator(json) {
  ...
  if (!hasKey) console.error('样式属性缺少“key”属性');
  if (!hasLabel) console.error('样式属性缺少“label”属性');
  if (!hasOptions) console.error('式属性缺少“options”属性或其不是数组');
  ...
  for (const option of json.options) {
    ...
    if (!hasIcon) console.error(`样式属性选项（${(_a = option.value) != null ? _a : option.label}) is missing the "icon" property`);
    if (!hasLabel2) console.error(`样式属性选项（${option.value}） is missing the "label" property`);
    if (!hasValue) console.error(`样式属性选项（${option.label}） is missing the "value" property`);
    if (!hasIcon || !hasLabel2 || !hasValue) optionsValid = false;
    if (option.value === null) hasDefault = true;
  }
  if (!hasDefault) console.error('样式属性缺少默认选项（值为 null 的选项）');
...
var SETTINGS = {
  general: {
    label: "常规",
    description: "高级画布插件的常规设置",
    disableToggle: true,
    children: {
      nodeTypeOnDoubleClick: {
        label: "双击时节点类型",
        description: "双击画布时将创建的节点类型",
        type: "dropdown",
        options: {
          "text": "文本",
          "file": "文件"
      ...
      alignNewNodesToGrid: {
        label: "始终将新节点与网格对齐",
        description: "启用时，新节点将与网格对齐。",
        type: "boolean"
      },
      defaultTextNodeDimensions: {
        label: "文本节点的默认尺寸",
        description: "文本节点默认的宽度和高度。",
        type: "dimension",
        parse: (value) => {
      ...
      minNodeSize: {
        label: "最小节点尺寸",
        description: "节点的最小尺寸（宽度或高度）。",
        type: "number",
        parse: (value) => Math.max(1, parseInt(value) || 0)
      },
      maxNodeWidth: {
        label: "最大节点宽度",
        description: "节点的最大宽度。设置为 -1 表示无限制。",
        type: "number",
        parse: (value) => Math.max(-1, parseInt(value) || 0)
      },
      disableFontSizeRelativeToZoom: {
        label: "禁用字体大小相对于缩放",
        description: "启用时，组节点标题和边标签的字体大小在缩小时不会增加。",
        type: "boolean"
      }
    }
  },
  commandsFeatureEnabled: {
    label: "扩展命令",
    description: "向画布添加更多命令。",
    infoSection: "canvas-commands",
    children: {
      zoomToClonedNode: {
        label: "缩放到克隆节点",
        description: "启用时，画布将缩放到克隆的节点。",
        type: "boolean"
      },
      cloneNodeMargin: {
        label: "克隆节点间距",
        description: "克隆节点与源节点之间的间距。",
        type: "number",
  ...
  canvasMetadataCompatibilityEnabled: {
    label: "启用 .canvas 元数据缓存兼容性",
    description: "使 .canvas 文件兼容反向链接和出站链接功能，并在图谱视图中显示连接。",
    infoSection: "full-metadata-cache-support",
    children: {
      treatFileNodeEdgesAsLinks: {
        label: "将文件节点之间的边视为链接",
        description: "启用后，文件节点之间的边将被视为链接。这意味着如果画布中文件节点 A.md 有一条边指向文件节点 B.md，文件 A.md 将在出站链接部分显示指向文件 B.md 的链接，并在图谱视图中显示连接关系。",
        type: "boolean"
      },
      enableSingleNodeLinks: {
        label: "启用支持使用 [维基链接]] 链接到节点",
        description: "启用后，您可以使用 [canvas-file#node-id]] 链接和嵌入节点。",
        type: "boolean"
      }
    }
  },
  nativeFileSearchEnabled: {
    label: "原生文件搜索",
    description: "启用后，文件搜索将使用 Obsidian 的原生文件搜索功能。",
    infoSection: "native-like-file-search",
    children: {}
  },
  autoFileNodeEdgesFeatureEnabled: {
    label: "自动创建文件节点连线",
    description: "根据文件节点的前置元数据链接，自动在它们之间创建连接。",
  ...
  portalsFeatureEnabled: {
    label: "传送门",
    description: "创建通往其他画布的传送门。",
    infoSection: "portals",
    children: {
      showEdgesIntoDisabledPortals: {
        label: "显示连接到禁用传送门的边",
        description: "启用时，连接到禁用传送门的边将显示。",
        type: "boolean"
      }
    }
  },
  collapsibleGroupsFeatureEnabled: {
    label: "可折叠组",
    description: "组节点可以折叠和展开，以保持画布的整洁。",
    infoSection: "collapsible-groups",
    children: {
      collapsedGroupPreviewOnDrag: {
        label: "拖动时折叠组预览",
        description: "启用时，折叠的组在拖动节点时会显示其边框。",
        type: "boolean"
      }
    }
  },
  combineCustomStylesInDropdown: {
    label: "合并自定义样式",
    description: "将高级画布的所有样式属性合并在一个下拉菜单中。",
    children: {}
  },
  nodeStylingFeatureEnabled: {
    label: "节点样式",
    description: "使用不同的形状和边框为节点设置样式",
    infoSection: "node-styles",
    children: {
      customNodeStyleAttributes: {
        label: "自定义节点样式设置",
        description: "为节点添加自定义样式设置。",
        type: "button",
  ...
  edgesStylingFeatureEnabled: {
    label: "边样式",
    description: "使用不同的路径样式为边设置样式。",
    infoSection: "edge-styles",
    children: {
      customEdgeStyleAttributes: {
        label: "自定义边样式设置",
        description: "为边添加自定义样式设置",
        type: "button",
        onClick: () => window.open("https://github.com/Developer-Mike/obsidian-advanced-canvas/blob/main/README.md#custom-styles")
      },
      ...
      edgeStyleUpdateWhileDragging: {
        label: "拖动时更新边样式（可能非常慢）",
        description: "启用时，在拖动边时，边的样式将会更新。",
        type: "boolean"
      },
      edgeStyleSquarePathRounded: {
        label: "方形路径圆角",
        description: "启用后，方形路径的边角将变为圆角。",
        type: "boolean"
      },
      edgeStylePathfinderAllowDiagonal: {
        label: "启用A* 对角线路径",
        description: "启用后，A* 路径样式将允许对角线路径。",
        type: "boolean"
      },
      edgeStylePathfinderPathRounded: {
        label: "A* 圆滑路径",
        description: "启用时，A* 路径样式将是圆滑的。",
        type: "boolean"
      }
    }
  },
  floatingEdgeFeatureEnabled: {
    label: "浮动边（自动边侧）",
    description: "浮动连接会自动放置在节点最合适的一侧。",
    infoSection: "floating-edges-automatic-edge-side",
    children: {
      allowFloatingEdgeCreation: {
        label: "允许创建浮动连接",
        description: "允许创建浮动连接 by dragging the edge over the target node without placing it over a specific side connection point. (If disabled, floating edges can only be created and used by other Advanced Canvas features.)",
        type: "boolean"
      },
      newEdgeFromSideFloating: {
        label: "从侧面浮动新建边",
        description: '启用后，边的“起始”端将始终浮动。',
        type: "boolean"
      }
    }
  },
  ...
  presentationFeatureEnabled: {
    ...
    children: {
      showSetStartNodeInPopup: {
        label: '在节点弹出窗口中显示“设置起始节点”',
        description: "如果关闭，您仍然可以使用相应的命令设置起始节点。",
        type: "boolean"
      },
      ...
      wrapInSlidePadding: {
        label: "包裹在幻灯片中的内边距",
        description: "将画布包裹在幻灯片中时，幻灯片的内边距。",
        type: "number",
        parse: (value) => Math.max(0, parseInt(value) || 0)
      },
      resetViewportOnPresentationEnd: {
        label: "演示结束时重置视口",
        description: "启用时，演示文稿结束后，视口将重置为原始位置。",
        type: "boolean"
      },
      useArrowKeysToChangeSlides: {
        label: "使用箭头键切换幻灯片",
        description: "启用时，您可以使用箭头键在演示模式下切换幻灯片。",
        type: "boolean"
      },
      usePgUpPgDownKeysToChangeSlides: {
        label: "使用 PgUp/PgDown 键切换幻灯片",
        description: "启用时，您可以使用 PgUp/PgDown 键在演示模式下切换幻灯片（使演示模式与大多数演示遥控器兼容）。",
        type: "boolean"
      },
      zoomToSlideWithoutPadding: {
        label: "缩放到没有内边距的幻灯片",
        description: "启用时，画布将缩放到没有内边距的幻灯片。",
        type: "boolean"
      },
      useUnclampedZoomWhilePresenting: {
        label: "在演示时使用未限制的缩放",
        description: "启用后，呈现时不会限制缩放。",
        type: "boolean"
      },
      ...
      slideTransitionAnimationDuration: {
        label: "幻灯片过渡动画持续时间",
        description: "幻灯片过渡动画的持续时间（以秒为单位）。设置为 0 以禁用动画。",
        type: "number",
        parse: (value) => Math.max(0, parseFloat(value) || 0)
      },
      slideTransitionAnimationIntensity: {
        label: "幻灯片过渡动画强度",
        description: "幻灯片过渡动画的强度。值越高，画布在缩放到下一个幻灯片之前，缩放得越多。",
        type: "number",
        parse: (value) => Math.max(0, parseFloat(value) || 0)
      }
    }
  ...
  zOrderingControlFeatureEnabled: {
    label: "Z 轴顺序控制",
    description: "通过右键菜单调整节点的固定Z轴层级。",
    children: {
      zOrderingControlShowOneLayerShiftOptions: {
        label: "显示一层偏移选项",
        description: "启用后，您可以将节点向前或向后移动一层。",
        type: "boolean"
      }
    }
  },
  aspectRatioControlFeatureEnabled: {
    label: "宽高比控制",
    description: "通过右键菜单调整节点的宽高比例。",
    children: {}
  },
  variableBreakpointFeatureEnabled: {
    label: "变量断点",
    description: `使用 ${VARIABLE_BREAKPOINT_CSS_VAR} CSS 变量，基于每个节点更改缩放断点（节点不再渲染内容的缩放级别）。`,
    infoSection: "variable-breakpoints",
    children: {}
  },
  autoResizeNodeFeatureEnabled: {
    label: "自动调整节点大小",
    description: "自动调整节点高度以适应内容",
    infoSection: "auto-node-resizing",
    children: {
      autoResizeNodeEnabledByDefault: {
        label: "默认启用自动调整大小",
        description: "启用时，所有节点默认启用自动调整大小功能。",
        type: "boolean"
      },
      autoResizeNodeMaxHeight: {
        label: "最大高度",
        description: "自动调整大小时节点的最大高度（-1表示无限制）。",
        type: "number",
        parse: (value) => {
          var _a;
  ...
  canvasEncapsulationEnabled: {
    label: "画布封装",
    description: "使用上下文菜单将选定的节点和边封装到一个新的画布中。",
  ...
  edgeHighlightEnabled: {
    label: "边缘高亮",
    description: "高亮选中节点的外向边缘（可选高亮内向边缘）。",
  ...
  createFeatureHeading(containerEl, label, description, infoSection, settingsKey) {
    ...
    if (infoSection !== void 0) {
      setting.addExtraButton(
        (button) => button.setTooltip("打开 GitHub 文档").setIcon("info").onClick(async () => {
    ...
    if (settingsKey !== null) {
      setting.addToggle(
        (toggle) => toggle.setTooltip("重载以生效").setValue(this.settingsManager.getSetting(settingsKey)).onChange(async (value) => {
          await this.settingsManager.setSetting({ [settingsKey]: value });
          new import_obsidian4.Notice("重新加载 Obsidian 以应用更改。");
        })
      );
    }
    return setting;
    ...
    Patcher.patchPrototype(this.plugin, view, {
      ...
      setViewData: Patcher.OverrideExisting((next) => function(json, ...args) {
        ...
        try {
          ...
          console.error("无效的 JSON，正在通过高级画布修复", e);
          if (this.file) that.plugin.createFileSnapshot(this.file.path, json);
...
var PresentationCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (menu, canvas) => {
        menu.addItem(
          (item) => item.setTitle("包裹在幻灯片中").setIcon("gallery-vertical").onClick(() => this.addSlide(
            canvas,
            void 0,
  ...
  async startPresentation(canvas, tryContinue = false) {
    if (!tryContinue || this.visitedNodeIds.length === 0) {
      ...
      if (!startNode2) {
        new import_obsidian13.Notice("未找到起始节点。请通过弹出菜单将一个节点标记为起始节点。");
        return;
      }
      ...
      try {
          ...
          if (item.type === CSSRule.IMPORT_RULE) {
            ...
            const deferred = fetchCSS(url).then((metadata) => embedFonts(metadata, options)).then((cssText) => parseCSS(cssText).forEach((rule) => {
              try {
                ...
                console.error("从远程CSS插入规则时出错", {
                  rule,
                  error
                });
              }
            })).catch((e) => {
              console.error("加载远程CSS失败", e.toString());
            });
        ...
        if (sheet.href != null) {
          deferreds.push(fetchCSS(sheet.href).then((metadata) => embedFonts(metadata, options)).then((cssText) => parseCSS(cssText).forEach((rule) => {
            ...
            console.error("远程样式表加载失败", err);
          }));
        }
        console.error("内联远程CSS文件时出错", e);
      }
    }
  });
    ...
    styleSheets.forEach((sheet) => {
      if ("cssRules" in sheet) {
        try {
          ...
          console.error(`从 ${sheet.href} 读取 CSS 规则时出错`, e);
        }
      }
    });
    return ret;
...
var ExportCanvasExtension = class extends CanvasExtension {
  ...
  async showExportImageSettingsModal(canvas, nodesToExport) {
    ...
    let svg = false;
    new import_obsidian18.Setting(modal.contentEl).setName("导出文件格式").setDesc("选择将画布导出为的文件格式。").addDropdown(
    ...
    pixelRatioSetting = new import_obsidian18.Setting(modal.contentEl).setName("像素比例").setDesc("更高的像素比例会生成更高分辨率的图像，但文件体积也会更大。").addSlider(
    ...
    noFontExportSetting = new import_obsidian18.Setting(modal.contentEl).setName("跳过字体导出").setDesc("这将不会在导出的 SVG 中包含字体。这会使 SVG 文件更小。").addToggle(
    ...
    new import_obsidian18.Setting(modal.contentEl).setName("Theme").setDesc("用于导出的主题。").addDropdown(
      (dropdown) => dropdown.addOptions({
        light: "Light",
        dark: "Dark"
    ...
    let watermark = false;
    new import_obsidian18.Setting(modal.contentEl).setName("显示徽标").setDesc("这将在左下角添加一个 Obsidian + 高级画布 的徽标。").addToggle(
      (toggle) => toggle.setValue(watermark).onChange((value) => watermark = value)
    );
    let garbledText = false;
    new import_obsidian18.Setting(modal.contentEl).setName("隐私模式").setDesc("这将隐藏画布上的所有文本。").addToggle(
      (toggle) => toggle.setValue(garbledText).onChange((value) => garbledText = value)
    );
    let transparentBackground = false;
    transparentBackgroundSetting = new import_obsidian18.Setting(modal.contentEl).setName("透明背景").setDesc("这将使图像的背景变为透明。").addToggle(
  ...
  async exportImage(canvas, nodesToExport, svg, pixelRatioFactor, noFontExport, theme, watermark, garbledText, transparentBackground) {
    ...
    new import_obsidian18.Notice("导出画布。请稍候...");
  ...
  getInteractionBlocker() {
    ...
    progressBarMessage.innerText = "生成图片...";
