...
1. 先将 `418122.xyz` 与 `www.418122.xyz` 的公网流量从 Vercel 切回旧 VPS 入口（例如恢复之前的 DNS 记录或代理目标）。
2. 确认 VPS 入口已指向可用前端容器（例如 Caddy 回指 `yohaku:2323`）。
3. 执行回退后验证：两域名访问 200、关键页面可打开、API 连通正常。
4. 记录回退时间点、原因和恢复计划，问题修复后再切回 Vercel 主路径。
